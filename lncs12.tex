...
Figure~\ref{fig:parameter} shows the influences of them on index size for our proposed methods when compressing both GOV2 and Common Crawl, from which we can gain an insight into these two parameters: $\varepsilon_{2}$ determines the sensitivity to partitions which contains exceptions, and $\varepsilon_{1}$ determines the largest partition length which contains no exceptions.
...
%		\renewcommand{\arraystretch}{1.4}
		...
		\begin{tabular}{*{3}c}
			...
			\midrule
			\textit{coarse} & \multirow{2}*{$ \left(0.03,0.7\right) $} & \multirow{2}*{$ \left(0.03,0.7\right) $} \\
			\textit{discreet} & & \\
			\bottomrule
			\label{tab: parameter}
		\end{tabular}
...
The last column denotes time used for a sequential decompression of the index, \textit{discreet} and \textit{coarse} tends to use 10\% longer time to traverse the index than optimal and uniform, delayed by suboptimal partitions.
...
\begin{table}
	...
	\begin{tabular}{@{}ll*{4}{r}}
		\toprule
		\multirow{2}*{collection} & \multirow{2}*{methods} & \multicolumn{1}{c}{compress} & \multicolumn{1}{c}{docid} & \multicolumn{1}{c}{freq} & \multicolumn{1}{c}{decompress} \\
		& &\multicolumn{1}{c}{\textbf{sec}} & \multicolumn{1}{c}{\textbf{bpi}} & \multicolumn{1}{c}{\textbf{bpi}} & \multicolumn{1}{c}{\textbf{sec}} \\
		\midrule
		\multirow{5}*{GOV2} & discreet & 870.70 & 4.40 & 2.32 & 99.46 \\
		...
		\midrule
		\multirow{5}*{Common Crawl} & discreet & 870.70 & 4.40 & 2.32 & 99.46 \\
		& coarse & 785.94 & 4.66 & 2.41 & 99.09 \\
		\cmidrule(l){2-6}
		& optimal & 11983.7 & 7.35 & 2.83 & 177.887 \\
